...
/*const deleteProduct =  async (req, res) => {
    try {
      ...
      // Use Mongoose to find and delete the document
      const deletedProduct = await Product.findOneAndDelete({ userID: userID, itemName: itemName });
...
const addBatchList = async (req, res) => {
  try {
    const { batchID, batchQty, expiryDate } = req.body; // Assuming the request body contains batchQty and expiryDate
